...
```{r}
keep.genes <- filterByExpr(y, group=y$samples$cluster, min.count=0.01, min.total.count=1)
table(keep.genes)
```
...
```{r}
for (i in 1:ncls){
  write.csv(topTags(qlf[[i]], n = Inf)$table, file=paste0('results/degenes_', gsub('/', '.', levels(cluster)[i]), '.csv'))
}
```
