...
annotation$cell_type <- factor(annotation$cell_type)
head(annotation)
```
...
```{r}
keep.genes <- filterByExpr(y, group=y$samples$cell_type, min.count=0.01, min.total.count=1)
table(keep.genes)
```
...
```{r}
cell_type <- as.factor(y$samples$cell_type)
plotMDS(y, col=cell_type, pch=1)
legend("bottomleft", legend=levels(cell_type), pch=1, col=c(2:18), cex=0.8, ncol=2)
```
...
design <- model.matrix(~ 0 + cell_type + roi)
...
for (i in 1:ncls){
  ...
  qlf[[i]]$comparison <- paste0(levels(cell_type)[i], '_vs_others')
}
```

The top most significant DE genes for cell_type L1
...
```{r}
for (i in 1:ncls){
  write.csv(topTags(qlf[[i]], n = Inf)$table, file=paste0('results/DE/degenes_', gsub('/', '.', levels(cell_type)[i]), '.csv'))
}
```
...
```{r}
lcpm <- cpm(y, log=TRUE)
annot <- data.frame(cell_type)
rownames(annot) <- colnames(y)
ann_colors <- list(cell_type=2:18)
names(ann_colors$cell_type) <- levels(cell_type)
pheatmap::pheatmap(lcpm[topMarkers,], breaks=seq(-2,2,length.out=101),
                   ...
                   cell_typeing_method="ward.D2", show_colnames=FALSE,
